...
export function ManageUploadsPanel({ password, refreshKey }: { password: string; refreshKey: number }) {
  ...
  async function handleSave(pathname: string) {
    const displayName = drafts[pathname] ?? "";
    ...
    <div className="flex flex-col gap-3">
      {items.map((item) => {
        ...
        return (
          <div
            key={item.pathname}
            className="flex flex-col gap-2 rounded-2xl border-[3px] border-black bg-cream-light p-4 sm:flex-row sm:items-center"
          >
            <span
            ...
            />
            <button
              onClick={() => handleSave(item.pathname)}
